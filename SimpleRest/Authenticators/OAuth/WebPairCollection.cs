...
namespace SimpleRest.Authenticators.OAuth
{
    internal class WebPairCollection : IList<WebPair>
    {
        ...
        public virtual void AddRange(Dictionary<string, string> collection)
        {
            AddCollection(collection);
        }

        private void AddCollection(Dictionary<string, string> collection)
        {
            var parameters = collection.Keys.Select(key => new WebPair(key, collection[key]));
